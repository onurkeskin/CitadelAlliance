...
#include "RecastDetourControl/fastlz.h"
...
struct MeshProcess : public dtTileCacheMeshProcess
{
	...
	inline void init(MeshPreparer* geom)
	{
		m_geom = geom;
	}
...
int NavMapController::rasterizeTileLayers(
...
void NavMapController::handleSettings()
{
...
void NavMapController::handleMeshChanged(class MeshPreparer* geom)
{
...
void NavMapController::addTempObstacle(const float* pos)
{
	if (!m_tileCache)
		return;
	float p[3];
...
void NavMapController::removeTempObstacle(const float* sp, const float* sq)
{
	if (!m_tileCache)
		return;
...
void NavMapController::clearAllTempObstacles()
{
	if (!m_tileCache)
		return;
...
bool NavMapController::handleBuild()
{
	dtStatus status;
...
void NavMapController::handleUpdate(const float dt)
{
...
void NavMapController::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void NavMapController::saveAll(const char* path)
{
...
void NavMapController::loadAll(const char* path)
{
	FILE *fp;
	errno_t err = fopen_s(&fp, path, "rb");
	if (fp == NULL)
