...
#include "RecastDetourControl/ChunkyTriMesh.h"
...
bool MeshPreparer::loadMesh(rcContext* ctx, const std::string& filepath)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
...
bool MeshPreparer::loadGeomSet(rcContext* ctx, const std::string& filepath)
{
	char* buf = 0;
	FILE *fp;
...
bool MeshPreparer::load(rcContext* ctx, const std::string& filepath)
{
...
bool MeshPreparer::saveGeomSet(const BuildSettings* settings)
{
...
bool MeshPreparer::raycastMesh(float* src, float* dst, float& tmin)
{
	float dir[3];
	rcVsub(dir, dst, src);
...
void MeshPreparer::addOffMeshConnection(const float* spos, const float* epos, const float rad,
...
void MeshPreparer::deleteOffMeshConnection(int i)
{
	m_offMeshConCount--;
...
void MeshPreparer::addConvexVolume(const float* verts, const int nverts,
...
void MeshPreparer::deleteConvexVolume(int i)
{
	m_volumeCount--;
